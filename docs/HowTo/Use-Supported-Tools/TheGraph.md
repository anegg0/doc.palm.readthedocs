...
3 - How to build a new subgraph and add it to the Palm network
...
Create subgraph.yaml: a Subgraph's manifest, it holds information about the smart contracts indexed by a Subgraph and is effectively a config file - More about this
Create schema.graphql: a GraphQL schema file defining a Subgraph’s structure - More about this
Create an AssemblyScript mapping — TypeScript code used to translate contracts’ event data into specific objects in the schema, The Graph offers a code generator for that- More about this
To get an idea of the usual structure of a subgraph repository you can use graphprotocol’s example subgraph for a start. For more NFT-specific repositories dedicated to EIP-721 or EIP-1155 NFTs.
...
How to add your own subgraph to the Palm network
